class Walker {
  ...
  void draw() { 
    ...
    float h = 0.6 - 0.5 * age / 1000.0;
    float s = constrain(1.0 - age / 1200.0, 0.0, 0.95);
    float b = constrain(0.2 + age / 750.0, 0.0, 0.95);
    fill( h, s, b );
    ellipse( location.x, location.y, radius * 2, radius * 2);
  }
  
  void drawFamilyTree(int depth) {
    float h = constrain( 0.9 - 0.3 * depth / 300.0, 0, 1.0);
    float s = constrain( 1.0 - depth / 100.0, 0, 1.0);
    float b = 0.95;
    stroke( h, s, b );
